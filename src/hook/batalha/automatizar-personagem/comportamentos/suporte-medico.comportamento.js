...
export function suporteMedicoComportamento(personagemAtivo, personagens) {
    let habilidadesBuffs = [...personagemAtivo.habilidades].filter(habilidade => habilidade.tipo === HABILIDADE_TIPO.BUFF)
    let habilidadesDebuffs = [...personagemAtivo.habilidades].filter(habilidade => habilidade.tipo === HABILIDADE_TIPO.DEBUFF)
        ...
        if(!alvoEscolhido) {
            ...
            if (acaoEscolhida.alvos === ALVOS.INIMIGOS_AREA || acaoEscolhida.alvos === ALVOS.ALIADOS_AREA) {
                ...
                if (acaoEscolhida.tipo.includes("ATAQUE")) {
                    ...
                    if(acaoEscolhida.tipo === HABILIDADE_TIPO.BUFF || acaoEscolhida.tipo === HABILIDADE_TIPO.DEBUFF) {
