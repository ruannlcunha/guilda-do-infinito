...
export function useAutomatizarPersonagem() {
    ...
    while (!acaoRealizada) {
      ...
      try {
        if (_verificarSePodeAgir(personagemAtivo)) {
          ...
          //Por enquanto BOTs não usam ações extras
          ...
          functions.ativarBannerInimigo(comportamento.acaoEscolhida.nome, personagemAtivo.perfil, comportamento.alvoEscolhido, personagemAtivo.corTema);
          await sleep(5100);
          await comportamento.acaoEscolhida.evento(personagemAtivo, comportamento.alvoEscolhido, comportamento.acaoEscolhida, functions, personagens);
          acaoRealizada = true;
        } else {
  ...
  function _verificarSePodeAgir(personagem) {
    if (personagem.condicoes.some((condicao) => condicao.nome === CONDICOES.PARALISADO.nome || condicao.nome === CONDICOES.DORMINDO.nome)) {
      return false;
    }
  ...
  function _realizarAcaoExtraDeCondicao(personagem) {
    ...
    if (personagem.condicoes.some((condicao) => condicao.nome === CONDICOES.CONGELADO.nome)) {
      acaoEscolhida = acoes.find((acao) => acao.id === ACOES_UNICAS.QUEBRAR_GELO.id);
    }
