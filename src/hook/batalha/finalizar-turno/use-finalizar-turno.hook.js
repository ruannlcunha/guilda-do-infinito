...
export function useFinalizarTurno() {
  ...
  function finalizarTurno(personagens, turnos, functions) {
    ...
    functions.setPersonagemAtivo(personagens.find((personagem) => personagem.idCombate === _idCombate));

    if (personagens.length > 0) {
      if (personagens.filter((item) => item.isInimigo === false).every((item) => item.isMorto)) {
        quantidadeDeJogadores === 2 ? _encerrarCombate("VITORIA DO JOGADOR 2", functions) : _encerrarCombate("DERROTA", functions);
      }

      if (personagens.filter((item) => item.isInimigo === true).every((item) => item.isMorto)) {
