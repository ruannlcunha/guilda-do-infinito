...
import { BONUS_DADO, CATEGORIA_CONDICAO, CONDICOES, ELEMENTOS, RESISTENCIA_DANO, TIPO_CONDICAO } from "../constants/personagens/personagem.constant";
import { validarProficiencia } from "./validacoes.util";
import { getDataEquipamento } from "./get-database.util";
...
export function instanciarPersonagem(_personagem) {
  ...
  const _equipamentos = _getEquipamentosProntos(personagem, data);
  ...
  const status = {
    pv: (data.status.pvBase + data.status.pvBonus * (personagemEvoluido.level - 1) + atributos.vigor * (personagemEvoluido.level - 1)) * personagem.multiplicadores.pv,
    pm: data.status.pmBase * personagemEvoluido.level * personagem.multiplicadores.pm
  };
  ...
  novosTalentos.map((talento) => {
    personagemInstanciado = talento.evento(personagemInstanciado, talento);
  });
...
function _getEquipamentosProntos(personagem, data) {
  const _equipamentos = personagem.equipamentoProntoId ? data.equipamentosProntos.find((equipamento) => equipamento.id === personagem.equipamentoProntoId) : personagem.equipamentos;

  return {
    ..._equipamentos,
    arma: validarProficiencia(getDataEquipamento(_equipamentos.arma), data) ? _equipamentos.arma : null,
    armadura: validarProficiencia(getDataEquipamento(_equipamentos.armadura), data) ? _equipamentos.armadura : null,
    acessorio1: validarProficiencia(getDataEquipamento(_equipamentos.acessorio1), data) ? _equipamentos.acessorio1 : null,
    acessorio2: validarProficiencia(getDataEquipamento(_equipamentos.acessorio2), data) ? _equipamentos.acessorio2 : null
  };
}
...
function _getAtaques(equipamentos, evolucao) {
  ...
  Object.entries(_getEquipamentos(equipamentos)).map((item) => {
    ...
    } else if (equipamentoNome === "arma") {
      ataquesEquipamentos = [...ataquesEquipamentos, { ataqueId: ATAQUES.GOLPE_DESARMADO.id, variantes: [] }];
    }
  });
  const ataquesObtidos = [...evolucao.ataques, ...ataquesEquipamentos];
  const _ataques = ATAQUES_DATA.filter((ataqueData) => ataquesObtidos.find((ataque) => ataque.ataqueId === ataqueData.id || ataque.ataqueId.id === ataqueData.id)).map((ataque) => {
...
function _getHabilidades(equipamentos, evolucao) {
  ...
  const _habilidades = HABILIDADES_DATA.filter((habilidadeData) => habilidadesObtidas.find((habilidade) => habilidade.habilidadeId === habilidadeData.id || habilidade.habilidadeId.id === habilidadeData.id)).map((habilidade) => {
    const variantesAtuais = habilidadesObtidas.find((habilidadeEvo) => habilidadeEvo.habilidadeId === habilidade.id || habilidadeEvo.habilidadeId.id === habilidade.id).variantes;
    ...
    if (_equipamento) {
      ...
      bonus.map((_bonus) => {
        if (Object.values(BONUS_DADO).some((bonus) => bonus === _bonus.atributo)) {
          const _novoBonus = {
            modificador: _bonus.valor,
            tipo: _bonus.atributo,
            atributo: _bonus.nome
          };
    ...
    if (_equipamento) {
      ...
      bonus.map((_bonus) => {
        if (Object.values(RESISTENCIA_DANO).some((bonus) => bonus === _bonus.atributo)) {
          ...
          const _novoBonus = {
            elemento: ELEMENTOS[elementoResistencia[1]],
            valor: _bonus.valor
          };
