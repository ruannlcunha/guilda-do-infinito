...
export function ganharExperiencia(personagem, expGanho) {
  ...
  if (experiencia > experienciaNecessaria) {
    while (experiencia > experienciaNecessaria) {
      ...
      experienciaNecessaria = PERSONAGENS_DATA.find((data) => data.id === personagem.personagemId).evolucoes.find((evolucao) => evolucao.level === novoPersonagem.level).experienciaNecessaria;
    }
  } else {
    novoPersonagem = {
      ...novoPersonagem,
