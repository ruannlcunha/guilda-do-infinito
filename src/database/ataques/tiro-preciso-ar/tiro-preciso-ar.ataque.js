...
export const TIRO_PRECISO_AR = {
    id: 71,
    nome: "Tiro Preciso (Ar)",
    dadoDeDano: "1d8+AGI+1d6(Ar)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um tiro de projétil de ar mirando com precisão no alvo.",
    elemento: ELEMENTOS.AR,
    categoria: CATEGORIAS_DE_DANO.DISTANCIA,
    custo: 0,
    evento: tiroPrecisoAr,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function tiroPrecisoAr(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], TIRO_PRECISO_AR.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, TIRO_PRECISO_AR, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.TIRO_AR, ACOES_AUDIO.FLECHA);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, TIRO_PRECISO_AR,
    )
  }