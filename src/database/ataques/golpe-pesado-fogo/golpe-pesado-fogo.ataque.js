...
export const GOLPE_PESADO_FOGO = {
    id: 37,
    nome: "Golpe Pesado (Fogo)",
    dadoDeDano: "1d10+FOR+1d6(Fogo)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe de fogo impactante causado por uma arma pesada.",
    elemento: ELEMENTOS.FOGO,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpePesadoFogo,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpePesadoFogo(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_PESADO_FOGO.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_PESADO_FOGO, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_PESADO_FOGO, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_PESADO_FOGO,
    )
  }