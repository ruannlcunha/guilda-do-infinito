...
export const GOLPE_PRECISO_MAGICO = {
    id: 25,
    nome: "Golpe Preciso (Mágico)",
    dadoDeDano: "1d8+FOR+1d6(Essência)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe mágico feito com precisão para acertar o inimigo.",
    elemento: ELEMENTOS.ESSENCIA,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpePrecisoMagico,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpePrecisoMagico(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_PRECISO_MAGICO.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_PRECISO_MAGICO, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_PRECISO_MAGICO, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_PRECISO_MAGICO,
    )
  }