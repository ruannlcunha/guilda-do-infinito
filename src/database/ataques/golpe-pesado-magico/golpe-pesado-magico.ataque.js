...
export const GOLPE_PESADO_MAGICO = {
    id: 35,
    nome: "Golpe Pesado (Mágico)",
    dadoDeDano: "1d10+FOR+1d6(Essência)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe mágico impactante causado por uma arma pesada.",
    elemento: ELEMENTOS.ESSENCIA,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpePesadoMagico,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpePesadoMagico(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_PESADO_MAGICO.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_PESADO_MAGICO, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_PESADO_MAGICO, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_PESADO_MAGICO,
    )
  }