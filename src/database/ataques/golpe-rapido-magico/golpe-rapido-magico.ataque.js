...
export const GOLPE_RAPIDO_MAGICO = {
    id: 45,
    nome: "Golpe Rápido (Mágico)",
    dadoDeDano: "1d6+FOR+1d6(Essência)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe mágico veloz que dificulta a esquiva do inimigo.",
    elemento: ELEMENTOS.ESSENCIA,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpeRapidoMagico,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpeRapidoMagico(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_RAPIDO_MAGICO.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_RAPIDO_MAGICO, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_RAPIDO_MAGICO, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_RAPIDO_MAGICO,
    )
  }