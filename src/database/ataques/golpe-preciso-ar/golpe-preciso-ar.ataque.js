...
export const GOLPE_PRECISO_AR = {
    id: 31,
    nome: "Golpe Preciso (Ar)",
    dadoDeDano: "1d8+FOR+1d6(Ar)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe de ar feito com precisão para acertar o inimigo.",
    elemento: ELEMENTOS.AR,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpePrecisoAr,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpePrecisoAr(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_PRECISO_AR.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_PRECISO_AR, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_PRECISO_AR, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_PRECISO_AR,
    )
  }