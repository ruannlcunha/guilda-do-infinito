...
export const GOLPE_PRECISO_FOGO = {
    id: 27,
    nome: "Golpe Preciso (Fogo)",
    dadoDeDano: "1d8+FOR+1d6(Fogo)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe de fogo feito com precisão para acertar o inimigo.",
    elemento: ELEMENTOS.FOGO,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpePrecisoFogo,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpePrecisoFogo(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_PRECISO_FOGO.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_PRECISO_FOGO, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_PRECISO_FOGO, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_PRECISO_FOGO,
    )
  }