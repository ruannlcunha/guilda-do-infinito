...
export const GOLPE_PRECISO_TERRA = {
    id: 30,
    nome: "Golpe Preciso (Terra)",
    dadoDeDano: "1d8+FOR+1d6(Terra)",
    tipo: ATAQUES_TIPO.ATAQUE_PURO,
    descricao: "Um golpe de terra feito com precisão para acertar o inimigo.",
    elemento: ELEMENTOS.TERRA,
    categoria: CATEGORIAS_DE_DANO.CORPO_A_CORPO,
    custo: 0,
    evento: golpePrecisoTerra,
    alvos: ALVOS.INIMIGOS,
    execucao: ACAO_EXECUCAO.PADRAO,
    variantes: [],
}

function golpePrecisoTerra(personagem, alvo, acao, functions) {
    ...
    const dado1d6Elemental = rolarDado(1, 6, [], GOLPE_PRECISO_TERRA.elemento, alvo.elemento)
    ...
    realizarEtapasAtaque(
      ...
      ()=>{
        const novoAlvo = causarDano(resultadoAtaque.alvo, danos, resultadoAtaque, GOLPE_PRECISO_TERRA, functions);
        const duracao = iniciarEfeito(novoAlvo, functions, EFFECTS.GOLPE_PRECISO_TERRA, ACOES_AUDIO.CORTE);
        finalizarAcao(functions, novoAlvo, duracao);
      },
      ()=>{
        finalizarAcao(functions, resultadoAtaque.alvo, 0);
      }, resultadoAtaque, functions, personagem, alvo, GOLPE_PRECISO_TERRA,
    )
  }