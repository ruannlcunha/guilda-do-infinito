  ...
  return (
    ...
    <div className="banner-rolagem" style={bannerStyle}>
      {banners.dados.map((dado, i) => {
        return (
          <>
            {i !== 0 && banners.dados.length > 1 ? <h2 key={i}>+</h2> : null}
            <div className="dado" key={`${i}-${dado.resultado}`}>
              {dado.elemento !== ELEMENTOS.FISICO ? (
                <div
                  className="elemento-dado"
                  style={{
                    backgroundImage: `url(${ICONS[`ELEMENTO_${dado.elemento}`]})`
                  }}
                >
          ...
          return (
            <>
              ...
              <div key={`${i}-${modificador.valor}`}>
                <div className={modificador.atributo}>{modificador.valor < 0 ? Math.abs(modificador.valor) : modificador.valor}</div>
