  ...
  return (
    <div className="logs-container" style={isMaximizado ? { height: "90%" } : null}>
      <header style={isMaximizado ? { height: "5%" } : null}>
        <button onClick={botaoMaximizar}>
          <img src={isMaximizado ? ICONS.MINIMIZAR : ICONS.MAXIMIZAR} alt="ícone de maximizar" />
        </button>
        <h1>Logs da batalha</h1>
        <button onClick={botaoLog}>
          <img src={ativo ? ICONS.LOGS_ATIVO : ICONS.LOGS_INATIVO} alt="ícone de maximizar" />
        </button>
      </header>
      {ativo ? (
