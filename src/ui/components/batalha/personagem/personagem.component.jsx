...
export function Personagem({ personagem, personagens, idAtivo, animacoes, acaoAtiva, functions }) {
  console.log(personagem);
  const porcentagemVida = calcularPorcentagem(personagem.pv.atual, personagem.pv.maximo);

  const isAlvo = acaoAtiva.alvos.some((item) => item.idCombate === personagem.idCombate && (!personagem.isMorto || (acaoAtiva.acao.alvos === ALVOS.ALIADOS_MORTOS && personagem.isMorto)) && animacoes.escolhendoAlvo);
  ...
  function handleEscolherAlvo() {
    if (isArea) {
      ...
      acaoAtiva.acao.evento(acaoAtiva.personagem, [...acaoAtiva.alvos], acaoAtiva.acao, functions, personagens);
    } else {
      functions.setAcaoEmAndamento(true);
      acaoAtiva.acao.evento(acaoAtiva.personagem, personagem, acaoAtiva.acao, functions, personagens);
    }
  }

  return (
    <div className={estaEscolhido ? "personagem-escolhido" : null} onClick={isAlvo ? handleEscolherAlvo : null} onMouseEnter={hoverEscolherAlvo} onMouseLeave={hoverEscolherAlvo}>
      {!animacoes.batalhaTerminou ? (
        <div className="icones-container">
          <img
            src={
              estaEscolhido || (isAlvo && isArea)
                ? ICONS.ESCOLHER_BAIXO
                : isAlvo && personagem.isInimigo
                ? ICONS.CRISTAL_VERMELHO
                : isAlvo && !personagem.isInimigo
                ? ICONS.CRISTAL_AZUL
                : estaAtivo && personagem.isInimigo && !animacoes.escolhendoAlvo
                ? ICONS.SETA_ATIVO_INIMIGO
                : estaAtivo && !animacoes.escolhendoAlvo
                ? ICONS.SETA_ATIVO
          ...
          <img
            src={
              isAlvo && acaoAtiva.tipoAcao === "Ataques" && ELEMENTOS_REACOES[acaoAtiva.acao.elemento].vantagem === personagem.elemento
                ? ICONS.SETA_VANTAGEM
                : isAlvo && acaoAtiva.tipoAcao === "Ataques" && ELEMENTOS_REACOES[acaoAtiva.acao.elemento].desvantagem === personagem.elemento
                ? ICONS.SETA_DESVANTAGEM
                : ICONS.QUADRADO_VAZIO
            }
          ...
          {personagem.testeResistencia ? (
            <div
              className="icone-dado-resistencia"
              style={{
                backgroundImage: `url(${personagem.isInimigo ? DADO_VERMELHO : DADO_AZUL})`
              }}
            >
              {personagem.testeResistencia}
            </div>
      ...
      <img src={personagem.sprite} alt="Sprite do personagem" style={personagemStyle(personagem, estaAtivo, isAlvo, animacoes.escolhendoAlvo, acaoAtiva)} className="sprite-personagem" />

      {personagem.effect.isAtivo ? <img src={personagem.effect.asset} alt="" className="acao-effect" /> : null}
