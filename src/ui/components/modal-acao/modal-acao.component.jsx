...
export function ModalAcao({ isOpen, setIsOpen, acao }) {
  ...
  useEffect(() => {
    if (acao) {
      if (acao.categoria) {
        acao.categoria === ACAO_CATEGORIA.CORPO_A_CORPO
          ? setCategoriaIcon(ICONS.FORCA)
          : acao.categoria === ACAO_CATEGORIA.DISTANCIA
          ? setCategoriaIcon(ICONS.AGILIDADE)
          : acao.categoria === ACAO_CATEGORIA.MAGICO
          ? setCategoriaIcon(ICONS.MAGIA)
          : setCategoriaIcon(ICONS.ELEMENTO_NULO);
      }
    }
  }, [acao]);
      ...
      <div className="modal-acao">
        ...
        <section className="acao-detalhes">
          ...
          <div>
            ...
            <ul>
              <li>
                <span>Custo:</span> {acao.custo} PM
              </li>
              <li>
                <span>Elemento:</span>
                <div className="nome-elemento" style={{ background: `var(--fundo-${acao.elemento})` }}>
                  {ELEMENTOS_NOMES[acao.elemento]}
                  <img src={ICONS[`ELEMENTO_${acao.elemento}`]} alt={`Símbolo do elemento ${ELEMENTOS_NOMES[acao.elemento]}`} />
                </div>
              </li>
              <li>
                <span>Alvos:</span>
                  ...
                  <li key={i}>
                    ...
                    <section>
                      {variante.lista.map((acaoVariante) => {
                        const novaAcao = {
                          ...acao,
                          ...acaoVariante.novaAcao,
                          descricao: acaoVariante.descricao
                        };
