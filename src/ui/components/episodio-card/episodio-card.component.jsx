...
export function EpisodioCard({item, selectedData, setSelectedData, onClick, isBloqueado, isConcluido}) {
    ...
    return (<>
            ...
            onMouseEnter={!isBloqueado?()=>{handleHover(item)}:null}
            onClick={!isBloqueado?()=>handleClick(item):null}
            style={selectStyle()}
            >
                <div className="preview-image"
                style={item.previewImage&&!isBloqueado ? {backgroundImage: `url(${item.previewImage})`}
                : {backgroundColor: "black"}}
                >
                    {isBloqueado?
                    <img src={ICONS.CADEADO} alt="" />
                    :null}
                </div>
                <section>
                    <h1>
                        {!isBloqueado?item.title:"???"}
                    </h1>
                    <section>
                        {
                        ...
                        <h1
                        style={item.tipo!=EPISODIO_TIPO.DIALOGO && !isBloqueado?
                            {
                ...
                {isConcluido?
                    <img src={ICONS.CONCLUIDO} alt="Episódio Concluído" className="concluido"/>
                :null}
            </li>
            </>
            )
