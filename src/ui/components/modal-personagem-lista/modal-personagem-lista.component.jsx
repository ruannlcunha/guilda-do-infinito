...
export function ModalPersonagemLista({modalIsOpen, setModalIsOpen, aliadosExtras,
                                        personagemTrocado, aliados, functions}) {
    ...
    function handleTrocar() {
        ...
        functions.setAliadosExtras(old=>{
            ...
            if(_aliadosExtras.find(aliado=>
            aliado.personagemId===personagemEscolhido.id && personagemEscolhido.isExtra)) {
                _aliadosExtras = old.filter(aliado=>
                    aliado.personagemId!==personagemEscolhido.id && personagemEscolhido.isExtra)
            }
    ...
    function renderCardPersonagem(personagem) {
        const escolhido = (personagemEscolhido.id===personagem.id) &&
        (personagem.isExtra===personagemEscolhido.isExtra)
        return (
            <li
            onMouseEnter={()=>playHover(1)}
            onClick={()=>{handleEscolher(personagem)}}
            className={escolhido?"card-escolhido":null}
            style={personagem.isExtra?
                {
                ...
                <header>
                    {personagem.elemento?
                        ...
                        className="elemento-icon"
                        />
                    :null}
                    {personagem.isExtra?
                        <img src={ICONS.ALIADO}
                        alt="Ícone de Aliado"
                        className="aliado-icon"
                        />
                    :null}
                </header>
        ...
        <div className="modal-personagem-lista">
            ...
            <section>
                ...
                <>
                    ...
                    ativo={(personagemTrocado.id!==personagemEscolhido.id
                        ||personagemTrocado.isExtra!==personagemEscolhido.isExtra)}
                    >
                        Selecionar
                    </BotaoPrimario>
                </>
