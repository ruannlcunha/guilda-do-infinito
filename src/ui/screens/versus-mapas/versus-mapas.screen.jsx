...
export function VersusMapaScreen() {
  ...
  function renderCardMapa(mapa, index) {
    return <li key={index} onMouseEnter={() => playHover(1)} className={mapa.id === mapaEscolhido.id ? "card-escolhido" : null} onClick={() => handleEscolherCard(mapa)} style={{ backgroundImage: `url(${mapa.img})` }}></li>;
  }
    ...
    <ContainerScreen>
      ...
      <div className="versus-mapas">
        ...
        <section className="menu-opcoes">
          <BotaoPrimario onClick={mapaEscolhido.id ? handleConfirmar : null} ativo={mapaEscolhido.id}>
            Confirmar
          </BotaoPrimario>
        ...
        <section className="mapas-list">
          ...
          <section>
            <button onMouseEnter={() => playHover(2)} onClick={handleVoltarPagina} className="setas">
              {pagina.min > 0 ? <img src={ICONS.SETA_DIREITA} alt="" style={{ transform: "scaleX(-1)" }} /> : null}
            </button>
            <ul>
              {mapas.map((mapa, i) => {
            ...
            <button onMouseEnter={() => playHover(2)} onClick={handleProximaPagina} className="setas">
              {pagina.max - 1 <= mapas.length ? <img src={ICONS.SETA_DIREITA} alt="" /> : null}
            </button>
          </section>
        </section>
      </div>
