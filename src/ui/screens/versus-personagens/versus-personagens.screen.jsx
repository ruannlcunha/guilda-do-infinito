...
export function VersusPersonagensScreen() {
  ...
  function setPaginaTotal() {
    ...
    setPagina((old) => {
      return { ...old, pagTotal: _pagina.pag };
    });
  }
  ...
  function handleFinalizarEquipe() {
    playClick(2);
    setPersonagemEscolhido(PERSONAGEM_VAZIO);
    equipeAtual === EQUIPES.ALIADOS ? setEquipeAtual(EQUIPES.INIMIGOS) : setEquipeAtual(EQUIPES.CONCLUIDO);
  }
  ...
  function renderEscolhidoNome(isAliado) {
    const _personagem =
      equipeAtual !== EQUIPES.ALIADOS && isAliado ? aliados[aliados.length - 1] : equipeAtual === EQUIPES.ALIADOS && !isAliado ? PERSONAGEM_VAZIO : equipeAtual === EQUIPES.CONCLUIDO ? inimigos[inimigos.length - 1] : personagemEscolhido;
  ...
  function renderEscolhidoPerfil(isAliado) {
    const _personagem =
      equipeAtual !== EQUIPES.ALIADOS && isAliado ? aliados[aliados.length - 1] : equipeAtual === EQUIPES.ALIADOS && !isAliado ? PERSONAGEM_VAZIO : equipeAtual === EQUIPES.CONCLUIDO ? inimigos[inimigos.length - 1] : personagemEscolhido;
    return <img src={_personagem.perfil} alt="" style={!isAliado ? { transform: "scaleX(-1)" } : null} />;
  }
  ...
  function renderPersonagemEscolhido(isAliado, personagem) {
    return (
      <div
        key={personagem}
        style={{
          background: `radial-gradient(circle, var(--black) 10%, var(--${personagem ? personagem.corTema : "tema-cinza"}) 100%)`,
          backgroundSize: "cover",
          backgroundPosition: "center"
        }}
      >
        <img style={!isAliado ? { transform: "scaleX(-1)" } : null} src={personagem ? personagem.perfil : IMAGES.GENERICO_PERFIL} alt="" />
      </div>
    );
  }
  ...
  function renderEquipamento(tipo, icon) {
    const item = EQUIPAMENTOS_DATA.find((item) => item.id === personagemEscolhido.equipamentos[tipo]);
    return (
      <div
        style={{
          cursor: `${item ? "var(--cursor-pointer)" : "var(--cursor-auto)"}`
        }}
        onClick={item ? () => handleDetalharItem(item) : null}
      >
        {item ? <img src={item.sprite} alt="" /> : <img src={icon} alt="" style={{ opacity: "15%" }} />}
      </div>
    );
  }

  function renderTitulo() {
    return equipeAtual === EQUIPES.ALIADOS && jogadores > 1
      ? "Escolha a equipe do Jogador 1"
      : equipeAtual === EQUIPES.ALIADOS
      ? "Escolha sua Equipe"
      : equipeAtual === EQUIPES.INIMIGOS && jogadores > 1
      ? "Escolha a equipe do Jogador 2"
      : equipeAtual === EQUIPES.INIMIGOS
    ...
    <ContainerScreen>
      ...
      <div className="versus-personagens">
        ...
        <section className="personagens-list">
          <section>
            <button onMouseEnter={() => playHover(2)} onClick={handleVoltarPagina} className="setas">
              {pagina.min > 0 && equipeAtual !== EQUIPES.CONCLUIDO ? <img src={ICONS.SETA_DIREITA} alt="" style={{ transform: "scaleX(-1)" }} /> : null}
            </button>
            <ul style={personagens.length <= 10 ? { alignItems: "self-start" } : null}>
            ...
            <button onMouseEnter={() => playHover(2)} onClick={handleProximaPagina} className="setas">
              {pagina.max - 1 <= PERSONAGENS_DATA.length && equipeAtual !== EQUIPES.CONCLUIDO ? <img src={ICONS.SETA_DIREITA} alt="" /> : null}
            </button>
          </section>
        </section>
        <section className="menu-opcoes">
          <BotaoPrimario onClick={aliados.length > 0 || inimigos.length > 0 ? handleDesfazerEscolha : null} ativo={aliados.length > 0 || inimigos.length > 0}>
            Desfazer
          </BotaoPrimario>

          <BotaoPrimario onClick={personagemEscolhido.id ? handleEscolherPersonagem : null} ativo={personagemEscolhido.id}>
            Escolher
          </BotaoPrimario>

          <BotaoPrimario
            onClick={(equipeAtual === EQUIPES.ALIADOS && aliados.length > 0) || (equipeAtual === EQUIPES.INIMIGOS && inimigos.length > 0) ? handleFinalizarEquipe : null}
            ativo={(equipeAtual === EQUIPES.ALIADOS && aliados.length > 0) || (equipeAtual === EQUIPES.INIMIGOS && inimigos.length > 0)}
          >
            Finalizar Equipe
          </BotaoPrimario>
          {equipeAtual !== EQUIPES.CONCLUIDO ? (
            <h2 className="paginacao">
              p.{pagina.pag}/{pagina.pagTotal}
            </h2>
          ) : null}
        </section>
        ...
        {personagemEscolhido.id ? (
          <Modal isOpen={personagemModal} setIsOpen={setPersonagemModal}>
            <div className="personagem-modal">
              <header>
                <h1 className="titulo-modal">Definir Personagem</h1>
              </header>
              <section>
                <section className="personagem-info">
                  <header>
                    ...
                    <div>
                      {personagemEscolhido.level > 1 ? (
                        <button onMouseEnter={() => playHover(1)} onClick={handleDiminuirLevel}>
                          -
                        </button>
                      ) : (
                      ...
                      {findPersonagemData().evolucoes.length > personagemEscolhido.level ? (
                        <button onMouseEnter={() => playHover(1)} onClick={handleAumentarLevel}>
                          +
                        </button>
                      ) : (
                        <button className="botao-vazio"></button>
                  ...
                  <h1>Atributos:</h1>
                  <ul>
                    {renderAtributo("PV", `${personagemEscolhido.pv.atual}/${personagemEscolhido.pv.maximo}`, ICONS.PV)}
                    ...
                    {renderAtributo("PM", `${personagemEscolhido.pm.atual}/${personagemEscolhido.pm.maximo}`, ICONS.PM)}
                  ...
                  <div className="equipamentos">
                    <header>
                      Equipamentos:
                      <div>
                        <button onClick={personagemEscolhido.equipamentoProntoId > 1 ? handleVoltarEquipamento : null} onMouseEnter={() => playHover(2)}>
                          {personagemEscolhido.equipamentoProntoId > 1 ? <img src={ICONS.SETA_DIREITA} alt="" style={{ transform: "scaleX(-1)" }} /> : null}
                        </button>
                        <h1>{personagemEscolhido.equipamentos.nome}</h1>
                        <button onClick={personagemEscolhido.equipamentoProntoId !== findPersonagemData().equipamentosProntos.length ? handleProximoEquipamento : null} onMouseEnter={() => playHover(2)}>
                          {personagemEscolhido.equipamentoProntoId !== findPersonagemData().equipamentosProntos.length ? <img src={ICONS.SETA_DIREITA} alt="" /> : null}
                        </button>
                      </div>
                    </header>
                    <section>
                ...
                <section className="personagem-visual">
                  <button onMouseEnter={personagemEscolhido.visualId > 1 ? () => playHover(2) : null} onClick={personagemEscolhido.visualId > 1 ? handleVoltarVisual : null}>
                    {personagemEscolhido.visualId > 1 ? <img src={ICONS.SETA_DIREITA} className="setas" alt="" style={{ transform: "scaleX(-1)", left: "100%" }} /> : null}
                  </button>
                  <div>
                    <h1>{personagemEscolhido.visualNome}</h1>
                    <img src={personagemEscolhido.sprite} alt="" />
                  </div>
                  <button onMouseEnter={findPersonagemData().visuais.length > personagemEscolhido.visualId ? () => playHover(2) : null} onClick={handleProximoVisual}>
                    {findPersonagemData().visuais.length > personagemEscolhido.visualId ? <img src={ICONS.SETA_DIREITA} className="setas" alt="" style={{ right: "100%" }} /> : null}
                  </button>
                </section>
              </section>
              <footer>
                <BotaoPrimario onClick={handleCancelarModal}>Cancelar</BotaoPrimario>
                <BotaoPrimario onClick={handleConfirmarPersonagem}>Confirmar</BotaoPrimario>
              </footer>
            </div>
          </Modal>
        ) : null}

        <ModalItem detalhesModal={itemModal} setDetalhesModal={setItemModal} itemEscolhido={itemEscolhido} />
      </div>
    </ContainerScreen>
  );
}
