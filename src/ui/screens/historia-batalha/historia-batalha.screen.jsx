...
export function HistoriaBatalhaScreen() {
  ...
  function fetchData() {
    ...
    const _aliados = _episodio.batalha.aliados.map((aliado) => {
      return { ...aliado, isExtra: true };
    });
  ...
  return personagensInstanciados.length > 0 && batalha ? (
    <>
      ...
      {telas.jogarBatalha ? (
        <JogarBatalha batalha={batalha} setMusica={setMusica} personagensInstanciados={personagensInstanciados} handleFinalizarBatalha={handleFinalizarBatalha} logsBatalha={logsBatalha} setLogsBatalha={setLogsBatalha} />
      ) : null}
      {telas.fimBatalha ? <FimDeBatalha batalha={batalha} personagens={personagensInstanciados} resultado={resultado} setMusica={setMusica} handleConcluirEpisodio={handleConcluirEpisodio} logs={logsBatalha} /> : null}
    </>
  ) : null;
}
