  ...
  useEffect(() => {
    if (selectedData) {
      if (selectedData.background) {
        document.documentElement.style.setProperty("--episodio-background", `url(${selectedData.background})`);
      } else {
        document.documentElement.style.setProperty("--episodio-background", `url()`);
      }
    } else {
          ...
          <section>
            <section className="capitulos-list-left">
              <section>
                <ul>
                  {capituloEscolhido ? (
                    <li className="sair-capitulo" onMouseEnter={hoverVoltarCapitulo} onClick={handleVoltarCapitulo}>
                  ...
                  {data && !capituloEscolhido ? (
                    data.capitulos.map((item) => {
                      ...
                      const capituloEncontrado = _historia ? _historia.capitulos.find((capitulo) => capitulo.capituloId === item.id) : { episodios: [] };
                      ...
                      return (
                        <CapituloCard
                          ...
                          onClick={!capituloEscolhido ? () => handleEscolherCapitulo(item) : () => {}}
                        />
                      );
                    })
                  ) : capituloEscolhido ? (
                    capituloEscolhido.episodios.map((item) => {
                      ...
                      const _historia = user.modos.historia.find((historia) => historia.historiaId === data.id);
                      const capituloEncontrado = _historia ? _historia.capitulos.find((capitulo) => capitulo.capituloId === capituloEscolhido.id) : { episodios: [] };
                      const _capitulo = capituloEncontrado ? capituloEncontrado : { episodios: [] };
                      ...
                      return (
                        <EpisodioCard
                          ...
                          selectedData={selectedData}
                          setSelectedData={setSelectedData}
                          onClick={!capituloEscolhido ? () => handleEscolherCapitulo(item) : null}
            ...
            <section className="capitulos-list-right" style={!selectedData ? { opacity: "0%" } : null}>
              {selectedData ? (
                <div
                  style={{
                    backgroundImage: `url(${selectedData.previewImage})`
                  }}
                >
                  <section>
