...
export function CosmosScreen() {
  ...
  useEffect(() => {
    startMusic(true);
    user.modos.cosmos ? setGachaAtual(GACHA_DATA.find((gacha) => gacha.id === user.modos.cosmos)) : null;
  }, []);
  ...
  function renderJogarButton(quantidade) {
    return (
      <BotaoPrimario onClick={() => navigate(`/cosmos/${gachaAtual.id}/${quantidade}`)}>
        <div>
          <img src={ICONS.BENCAO} alt="Benção de Orion" />x{quantidade}
        </div>
  ...
  function renderBannerOpcao(gacha) {
    return (
      <li>
        <div onMouseEnter={() => playHover(1)} onClick={() => handleEscolherGacha(gacha.id)} className={gacha.id === gachaAtual.id ? "opcao-escolhida" : "opcao"} style={{ backgroundImage: `url(${gacha.preview})` }}>
