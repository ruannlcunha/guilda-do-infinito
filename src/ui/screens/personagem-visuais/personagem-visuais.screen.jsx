...
export function PersonagemVisuaisScreen() {
  ...
  useEffect(() => {
    if (user) {
      ...
      setVisuais(
        personagemData.visuais.map((visual) => {
  ...
  function handleEquiparVisual() {
    ...
    const novosPersonagens = [...user.personagens.filter((personagem) => personagem.personagemId != personagemId), novoPersonagem];
  ...
  function renderVisualFundo(lado, modificador) {
    return (
      <div style={{ [lado]: "10%" }} className="visual-fundo">
        <img src={visuais[visualIndex + modificador].sprite} alt="Sprite do visual" className={visuais[visualIndex + modificador].bloqueado ? "visual-bloqueado" : null} />
      </div>
    );
  }
  ...
  function renderVisualPrincipal() {
    return (
      <div className="visual">
        <img src={visuais[visualIndex].sprite} alt="Sprite do visual" className={visuais[visualIndex].bloqueado ? "visual-bloqueado" : null} />
        ...
        <h1>{!visuais[visualIndex].bloqueado ? visuais[visualIndex].nome : "???"}</h1>
      </div>
    );
  }
      ...
      <div className="personagem-visuais">
        ...
        {visuais.length > 0 ? (
          <section className="visuais-section">
            ...
            <BotaoPrimario onClick={handleEquiparVisual} ativo={personagem.visualId !== visuais[visualIndex].visualId && !visuais[visualIndex].bloqueado}>
              Equipar
            </BotaoPrimario>
          </section>
        ) : null}
