...
export function PerfilPersonagensScreen() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const personagensInstanciados = _personagens
        .map((item) => {
  ...
  useEffect(() => {
    if (personagemEscolhido.id !== 0) {
      document.documentElement.style.setProperty("--fundo-tema", `var(--${personagemEscolhido.corTema})`);
    } else {
      document.documentElement.style.setProperty("--fundo-tema", `var(--blue-grey)`);
    }
  }, [personagemEscolhido]);
  ...
  function renderCardPersonagem(personagem) {
    return (
      <li
        ...
        style={{
          background: `url(${personagem.perfil}), var(--card-${personagem.raridade}-estrelas)`,
          backgroundSize: "contain",
          backgroundPosition: "center",
          backgroundRepeat: "no-repeat"
        }}
      >
        <header>{personagem.elemento ? <img src={ICONS[`ELEMENTO_${personagem.elemento}`]} alt="Ícone do Elemento" /> : null}</header>
      ...
      <div className="perfil-personagens">
        ...
        {personagemEscolhido.id !== 0 ? (
          <section className="personagem-selecionado">
            ...
            <BotaoPrimario onClick={handleVisualizar}>Visualizar</BotaoPrimario>
          </section>
        ) : null}
      </div>
    </ContainerScreen>
