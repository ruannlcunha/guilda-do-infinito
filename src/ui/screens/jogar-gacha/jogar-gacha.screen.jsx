...
export function JogarGacha() {
    ...
    useEffect(()=>{
        ...
        if(user.bencaos >= quantidade) {
            ...
            const _cena2Sound = setTimeout(()=>{setCenaId(2)},5500)
    ...
    useEffect(()=>{
        if(cenaId===2) {
            ...
            const _gacha4Sound = setTimeout(()=>{playSound(SOUNDS.GACHA_4)},4000)
    ...
    function instanciarRecompensas(banner) {
        ...
        for(let i=0;i<quantidade;i++) {
            ...
            const possibilidadeEscolhida = banner.possibilidades.find(
        ...
        recompensasObtidas.map(item=> {
            ...
            if(item.tipo===RECOMPENSAS_TIPO.PERSONAGEM) {
                if(!novosPersonagens.find(personagem=> personagem.personagemId==item.recompensa.id)) {
                    const _personagem = {...personagemBase, personagemId: item.recompensa.id,}
    ...
    function handleContinuar() {
        playClick(2)
        timeoutId.map(soundId=>{clearTimeout(soundId)})
        if(cenaId===3) {
            navigate(-1)
        }
                    ...
                    style={{backgroundImage:`url(${recompensas[recompensaAtualId].recompensa.santuario})`}}>
                        <div className="gacha-detalhes">
                            ...
                            <div className="estrelas">
                                {renderEstrelas(recompensas[recompensaAtualId])}
                            </div>
                            <h2>{recompensas[recompensaAtualId].tipo === RECOMPENSAS_TIPO.ARMA ?
                            recompensas[recompensaAtualId].recompensa.tipo :
                            recompensas[recompensaAtualId].recompensa.titulo
                            }</h2>
                        </div>
                            ...
                            return (
                                ...
                                url(${item.brilho})`}}>
                                    ...
                                    <div className="gacha-detalhes">
                                        ...
                                        <h2>{item.tipo === RECOMPENSAS_TIPO.ARMA ?
                                        item.recompensa.tipo :
    ...
    return (
        <ContainerScreen>
            ...
            <div className="jogar-gacha-screen">
                ...
                <button className="pular-button" onClick={handlePular}>Pular</button>
                :null}
                ...
                {cenaId===1 && possuiBencao ?
                renderCena1()
                :null}

                {cenaId===2 && possuiBencao ?
                renderCena2()
                :null}
