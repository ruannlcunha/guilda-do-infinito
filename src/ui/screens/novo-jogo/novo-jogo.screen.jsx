...
export function NovoJogoScreen() {
    ...
    const DIALOGOS = novoJogoCenas(formData.visualId, formData.pronomes, formData.nome,
    ...
    function renderVisualOption(id, sprite) {
        return (
            ...
            style={formData.visualId===id ? {
                backgroundColor: "var(--light-grey)",
                border: "solid 2px var(--blue)",
            }:null}
            onClick={()=>escolherVisual(id)}>
                <img
                style={formData.visualId===id ? {width: "100%"}:null}
                src={sprite}
                alt={`Sprite do Visual ${id}`}
                />
                <h2>Visual {id}</h2>
            </li>
        )
    ...
    function renderPronomeOption(pronomes) {
        return (
            <li onMouseEnter={()=>playHover(1)}
            style={formData.pronomes===pronomes ? {
                backgroundColor: "var(--light-grey)",
                border: "solid 2px var(--blue)",
            }:null}
        ...
        <ContainerScreen>
            ...
            <Modal isOpen={visualModal} setIsOpen={()=>{}}>
                <div className="visual-modal">
                    ...
                    <BotaoPrimario onClick={confirmarVisual}>
                        Escolher
                    </BotaoPrimario>
                </div>
            </Modal>
            ...
            <Modal isOpen={pronomesModal} setIsOpen={()=>{}}>
                <div className="pronomes-modal">
                    ...
                    <BotaoPrimario onClick={confirmarPronomes}>
                        Escolher
                    </BotaoPrimario>
                </div>
            </Modal>
            ...
            <Modal isOpen={confirmarModal} setIsOpen={()=>{}}>
                <div className="confirmar-modal">
                    ...
                    <ul>
                        <div>
                            <img src={AVENTUREIRO_SPRITE[`VISUAL_${formData.visualId}_SPRITE`]}
                            alt="Sprite do aventureiro" />
                            <li style={{padding: "0"}}>
                                <h2 style={{textAlign: "center"}}>
                                Visual {formData.visualId}
                                </h2>
                            </li>
                        </div>
                        <div>
                            <li><h2><span>Nome:</span> {formData.nome}</h2></li>
                            <li><h2><span>Pronomes:</span> {formData.pronomes.pronome}</h2></li>
                        </div>
                    </ul>
                    <div>
                    <BotaoPrimario onClick={handleCriarUsuario}>
                        Confirmar
                    </BotaoPrimario>
                    <BotaoPrimario onClick={handleRecomecar}>
                        Refazer
                    </BotaoPrimario>
                    </div>
                </div>
